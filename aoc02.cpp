...
// In:  Range<Range<T>>
template<class Rngs>
auto checksum_part2(Rngs&& x) {
    std::vector<int> result;
    for_each(x, [&](auto x) {
        auto aaa = view::cartesian_product(x, x);
        for_each(aaa, [&](auto y) {
            auto a = std::get<0>(y);
            auto b = std::get<1>(y);
            if (a == b) {}
            else if ((b % a) == 0) result.push_back(b / a);
        });
    });
...
int main()
{
    const std::vector<std::vector<int>> test01 = {
        {5, 1, 9, 5},
        {7, 5, 3},
        {2, 4, 6, 8}
    };

    const std::vector<std::vector<int>> test02 = {
    ...
    assert_eq(checksum_part1(test01), 18);
    assert_eq(checksum_part1(test02), 37923);

    const std::vector<std::vector<int>> test03 = {
        {5, 9, 2, 8},
        {9, 4, 7, 3},
        {3, 8, 6, 5}
    };

    assert_eq(checksum_part2(test03), 9);
    assert_eq(checksum_part2(test02), 263);
